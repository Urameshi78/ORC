...
int     ImageProcessor::processSet(std::string const &baseDir, QString outputFile){
  DIR * rep = opendir(baseDir.c_str());
  SetWriter writer(outputFile);

  if (writer.fileOutputQ.open(QIODevice::ReadWrite)) {
    QTextStream stream(&writer.fileOutputQ);
	...
	if (strcmp(ent->d_name, ".") != 0 && strcmp(ent->d_name, "..") != 0) {
	  std::cout << "Lecture des fichiers en cours : " << ent->d_name << std::endl;
	  int nb = processSample(baseDir + "/" + ent->d_name, writer, stream);
	  if (nb == 0)
	    return 0;
	  count += nb;
	}
...
int     ImageProcessor::processSample(std::string const &baseDir, SetWriter & writer,  QTextStream &stream){
  ...
  for (int value = 65; value <= 126; ++value){
    ...
    writer.addMatToSet(img, value - 32, stream);
  }
  return count;
}
